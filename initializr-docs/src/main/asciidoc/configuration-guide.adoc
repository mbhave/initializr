...
Project generation occurs in a dedicated application context which is a `ProjectGenerationContext`.
The `ProjectGenerationContext` imports `@ProjectGenerationConfiguration`-annotated
...
The components available in a `ProjectGenerationContext` are decided based on a `ProjectDescription`.
A `ProjectDescription` consists of the following project properties:
...
This walkthrough of how to create your own service assumes that the service will be used
